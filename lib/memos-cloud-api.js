...
export function buildConfig(pluginConfig = {}) {
  ...
  const readBaseUrl = cfg.readBaseUrl || loadEnvVar("MEMOS_READ_BASE_URL") || "";
  const writeBaseUrl = cfg.writeBaseUrl || loadEnvVar("MEMOS_WRITE_BASE_URL") || "";
  ...
  return {
    ...
    readBaseUrl: readBaseUrl ? readBaseUrl.replace(/\/+$/, "") : "",
    writeBaseUrl: writeBaseUrl ? writeBaseUrl.replace(/\/+$/, "") : "",
    apiKey,
    userId,
    conversationId,
    ...
    maxRecallItemChars: cfg.maxRecallItemChars ?? 220,
    maxPromptChars: cfg.maxPromptChars ?? 3600,
    recallEnabled: cfg.recallEnabled !== false,
    // Fail closed by default: no automatic turn capture unless explicitly enabled.
    addEnabled: cfg.addEnabled === true,
    captureStrategy: cfg.captureStrategy ?? "last_turn",
    capturePolicy: cfg.capturePolicy ?? "disabled",
    capturePrefix: cfg.capturePrefix ?? "MEMORY:",
    enforceQualityGate: cfg.enforceQualityGate !== false,
    minFactChars: cfg.minFactChars ?? 25,
    maxFactChars: cfg.maxFactChars ?? 500,
    maxMessageChars: cfg.maxMessageChars ?? 20000,
    includeAssistant: cfg.includeAssistant === true,
    ...
    info: cfg.info ?? {},
    includeInfo: cfg.includeInfo === true,
    agentId: cfg.agentId,
    appId: cfg.appId,
    ...
    localMode: parseBool(cfg.localMode, parseBool(loadEnvVar("MEMOS_LOCAL_MODE"), false)),
    readableCubeIds: cfg.readableCubeIds ?? [],
    writableCubeIds: cfg.writableCubeIds ?? [],
    cubeRouting: cfg.cubeRouting ?? {},
  };
}
...
export async function callApiLocal(
  { baseUrl, readBaseUrl = "", writeBaseUrl = "", timeoutMs = 5000, retries = 1 },
  path,
  body,
) {
  let lastError;
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      ...
      // Normalize local MemOS response to match cloud format
      if (path === "/product/search" && json.data) {
        const d = json.data;
        const memoryDetailList = [];
        const preferenceDetailList = [];
        const toolMemoryDetailList = [];

        for (const mem of d.text_mem ?? []) {
          for (const m of mem.memories ?? []) {
            memoryDetailList.push({
              memory_key: m.memory_key ?? m.memory ?? "",
              memory_value: m.memory_value ?? m.memory ?? "",
              create_time: m.create_time,
            });
          }
        }
        for (const mem of d.preference_mem ?? d.pref_mem ?? []) {
          for (const m of mem.memories ?? []) {
            preferenceDetailList.push({
              preference: m.preference ?? m.memory ?? "",
              preference_type: m.preference_type,
              create_time: m.create_time,
            });
          }
        }
        for (const mem of d.tool_mem ?? []) {
          for (const m of mem.memories ?? []) {
            toolMemoryDetailList.push({
              tool_value: m.tool_value ?? m.memory ?? "",
              create_time: m.create_time,
            });
          }
        }
...
export async function searchMemory(cfg, payload) {
  if (cfg.localMode) return callApiLocal(cfg, "/product/search", payload);
  return callApi(cfg, "/search/memory", payload);
}

export async function addMessage(cfg, payload) {
  if (cfg.localMode) return callApiLocal(cfg, "/product/add", payload);
...
function buildPromptFromData(data, options = {}) {
  ...
  const lines = [
    "# MemOS Recall Context",
    "",
    `(Retrieved ${nowText}. Treat as supplemental context — your SOUL.md, AGENTS.md, and local memory files remain authoritative.)`,
    "",
    ...wrapCodeBlock(memoriesBlock, options),
    "",
    "Notes on recalled memories: entries tagged '[assistant观点]' or '[模型总结]' are past AI inference, not direct user statements. Verify relevance and freshness before using. Current user message follows.",
    "",
    USER_QUERY_MARKER,
  ];
